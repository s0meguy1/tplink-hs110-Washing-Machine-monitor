...
var monitoredDevice = {
    ...
    getTimeSinceLastStart: function () { return utils.getDate() - monitoredDevice.lastStartedTime; },
    getTimeFromLastRunningAlert: function () { return utils.getDate() - monitoredDevice.lastTimeRunningAlert; },
    getTimeFromLastIdleAlert: function () { return utils.getDate() - monitoredDevice.lastTimeIdleAlert; },
    ...
    if (monitoredDevice.getPower() > CONFIG.powerThreshold) {
        if (monitoredDevice.isDeviceStopped()) {
            ...
            if (CONFIG.enableStartAlert == "on") {
                ...
                utils.sendNotification(CONFIG.aliasDevice + " Started", api);
            }
        }
        ...
        if (CONFIG.enableStopAlert == "on") {
            ...
            utils.sendNotification(CONFIG.aliasDevice + " stopped", api);
        }
    }
}
...
function verifyIdleTime() {
    if (CONFIG.enableIdleAlert == "on" &&
        ...
        utils.sendNotification(CONFIG.aliasDevice + " didn't start for the last " + (monitoredDevice.getTimeSinceLastStart() / 60) + " minutes", api);
...
function verifyRunningTime() {
    if (CONFIG.enableRunningAlert == "on" &&
        ...
        utils.sendNotification(CONFIG.aliasDevice + " running for more then " + (monitoredDevice.getTimeSinceLastStart() / 60) + " minutes", api);
